...
/** TravelAgency for the Rhodes cruise planning brand. */
export function travelAgencySchema() {
  return {
    ...
    knowsAbout: [
      ...
      "Lindos from Rhodes cruise port",
      "Rhodes cruise port guide",
      "Rhodes Old Town from cruise port",
      "Rhodes beaches and coast",
      "Rhodes cruise planner",
    ],
    contactPoint: {
      "@type": "ContactPoint",
      email: SITE.email,
...
export function itemListSchema(items: { name: string; url: string }[]) {
  return {
    ...
    itemListElement: items.map((item, index) => ({
      ...
      position: index + 1,
      name: item.name,
      url: absoluteUrl(SITE.url, item.url),
    })),
  };
}
